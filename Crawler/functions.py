...
def extract_form_attrs(form):
    ...
    #generators ftw! ps: you used a generator cause the other way required 4 lines. no other reason.
...
def process_input(input,params_list):
    ...
    #using .get() to check for existence, as the dictionary if directly accessed will throw a keyerror
    name=''
    type=''
    value=''
    ...
    if(input.get('value')!=None):
        value=input['value']
    #no need to check if it has a name, if the inp object got passed, it will have a name. Period.
    elt_type=input.name
...
def check_input(input,value):
    ...
    if(input.get('name')!=None):
        ...
        return (re.search(search_string,input.get('name'))!=None)
    return False
    #returns true if the input field has name or id set to be the 'value' provided, else returns false.
    pass